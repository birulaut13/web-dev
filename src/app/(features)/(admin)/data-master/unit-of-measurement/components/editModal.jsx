...
export default function EditModal({
  ...
  const [newUnitOfMeasurementName, setNewUnitOfMeasurementName] = useState(unitOfMeasurement);
  const [unitOfMeasurementId, setUnitOfMeasurementId] = useState(id);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/uom/update/${unitOfMeasurementId}`,
        {
          method: "PUT",
          headers: {
  ...
  useEffect(() => {
    setOpen(openModal);
    setNewUnitOfMeasurementName(unitOfMeasurement);
    setUnitOfMeasurementId(id);
  }, [openModal, unitOfMeasurement, id]);
                ...
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center w-full sm:mt-0 sm:text-left">
                    ...
                    <div className="mt-2">
                      <input
                        type="text"
                        value={newUnitOfMeasurementName}
                        onChange={(e) => setNewUnitOfMeasurementName(e.target.value)}
